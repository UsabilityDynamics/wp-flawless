 ...
 * ========================================================= */
  ...
  jQuery( document ).ready( function() {
    ...
    jQuery( ".flawless_settings_tabs" ).tabs( {
      cookie: { expires: 30 }
    });
    ...
    flawless_adjust_dom_to_conditionals( 'instant' );

    jQuery( ".sets_conditional" ).change( function( e ) {
      flawless_adjust_dom_to_conditionals( 'instant', e );
    });
    ...
    jQuery( document ).bind( 'flawless::widget_area_change', function() {
      ...
      flawless_admin.widget_areas = {};

      jQuery( '.widget_area_sidebar .flawless_widget_item input.flawless_wa[type="text"]' ).each( function( index ) {
        flawless_admin.widget_areas[ jQuery( this ).closest( '.flawless_widget_item' ).attr( 'sidebar_name' ) ] = jQuery( this ).val();
      });

      /*
      jQuery( '.flawless_live_data_target[flawless_live_data_target=widget_areas]' ).each( function() {
        var select = jQuery( this );
        jQuery.each( flawless_admin.widget_areas, function( name, label ) {
          if( !jQuery( 'option[value=' + name + ']', select ).length ) {
            jQuery( select ).append( jQuery( '<option name="">' + label + '</option>' ) );
          }
        });
      });
      */
  ...
  function flawless_render_widget_areas_in_dom() {
    ...
    jQuery( '.flawless_add_new_widget_area' ).click( function() {
      ...
      /* Clone a flawless item, there should always be at lesat one */
  ...
  function flawless_adjust_dom_to_conditionals( type, e ) {
    ...
    jQuery( '.sets_conditional' ).each( function() {
      ...
      if( jQuery( result_element ).hasClass( 'ui-tabs-selected' ) ) {

        /*
        Need to somehow select a visible tab if the tab we disabled an active tab.

        var parent = jQuery( result_element ).closest( '.ui-tabs' );
        var available_tab = jQuery( '.ui-tabs-nav li.ui-state-default:visible:first', parent ).index();

        if( available_tab >= 0 ) {
          parent.tabs( 'select', available_tab );
        }
