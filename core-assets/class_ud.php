...
/**
 *
 * Name: Usability Dynamics UI and Functions
 * Description: The UD Class ported over for the Flawless theme.
 ...
 * Example:
 * <code>
 * echo Flawless_UI::checkbox("name=send_newsletter&group=settings&value=true&label=If checked, you will receive our newsletters.", $send_newsletter);
 * </code>
 *
 * Copyright 2010 Andy Potanin <andy.potanin@twincitiestech.com>
 *
...
<?php
 ...
 function checked_in_array($item, $array) {
  ...
  function checkbox($args = '', $checked = false) {
    ...
    $prefix = (defined('UD_PREFIX') ? UD_PREFIX : '');

    // Get rid of all brackets
    if(strpos("$name",'[') || strpos("$name",']')) {

      $class_from_name = trim($name);
      ...
      $class_from_name = $prefix . str_replace($replace_variables, '_', $class_from_name);
    } else {
      $class_from_name = $prefix . $name;
    }
    ...
    $insert_class     =   " class='$class_from_name $class ".$prefix."checkbox " . ($group ? $prefix . $group . '_checkbox' : ''). "' ";
  ...
  function print_admin_scripts($args = '') {
    ...
    <script type="text/javascript">
      ...
      function ud_json_processing(data) {
        ...
        // Show and print message
        jQuery(message_element).show();
        jQuery(message_element).html("<p><?php _('Processing...','flawless') ?></p>");
  ...
  function use_ud_scripts() {
    add_action('admin_head', array('Flawless_UI', 'print_admin_scripts'));
  }
  ...
  function show_log_page() {
    ...
    <div class="wrap">
    <h2><?php _e('UD Log Page for','flawless') ?> get_option('<?php echo UD_PREFIX . 'log'; ?>');
    <a href="<?php echo admin_url("admin.php?page=ud_log&ud_action=clear_log"); ?>" class="button"><?php _e('Clear Log','flawless') ?></a>
    </h2>
    ...
    <table class="widefat">
      <thead>
      <tr>
        <th style="width: 150px"><?php _e('Timestamp','flawless') ?></th>
        <th><?php _e('Type','flawless') ?></th>
        <th><?php _e('Event','flawless') ?></th>
        <th><?php _e('User','flawless') ?></th>
        <th><?php _e('Related Object','flawless') ?></th>
      </tr>
      </thead>
 ...
 *
 * @version 1.5
 * @link http://usabilitydynamics/codex/Flawless_F
 * @package UsabilityDynamics
 */
...
class Flawless_F {
  ...
  /**
   * Unserialize and check for bad encoding
   *
   */
  function maybe_unserialize( $original ) {
    ...
    $fixed = preg_replace_callback(
      '!(?<=^|;)s:(\d+)(?=:"(.*?)";(?:}|a:|s:|b:|i:|o:|N;))!s',
      array('Flawless_F', 'serialize_fix_callback'),
      $original );
  ...
  **/
  function objectToArray($object) {
      ...
      return array_map(array('Flawless_F' , 'objectToArray'), $object );
   }
  ...
  **/
   ...
   static function array_merge_recursive_distinct () {
    ...
    foreach($append as $key => $value) {
      ...
      $base[$key] = Flawless_F::array_merge_recursive_distinct($base[$key], $append[$key]);
  ...
  **/
  function add_cache($id = false, $type = false, $data = false) {
    ...
    if(Flawless_F::check_cache_table()) {
  ...
  **/
  function check_cache_table($table_name = 'ud_cache') {
    ...
    Flawless_F::log(__("Cache table does not exist. Attempting to create:",'flawless'));
    ...
    Flawless_F::log(sprintf(__('SQL Ran: %s , verifying existance.','flawless'), $sql ));


    // Verify it exists
    if($wpdb->get_var("SHOW TABLES LIKE '$table_name'") == $table_name) {
      Flawless_F::log(__("Table created and exists.",'flawless'));
      return true;
    }

    Flawless_F::log(__("Table not created.",'flawless'));
   ...
   * Example:
   * <code>
   * Flawless_F::log("Settings updated");
   * </code>
   *
   ...
   */
    ...
    if(!is_array($this_log)) {
      $this_log = array();
      array_push($this_log, array(time(), __("Log Started.",'flawless')));
    }
   ...
   * @uses strtotime()
   * @uses check_prefix()
   * @uses apply_filters() Calls 'Flawless_F_days_since' filter on provided date.
  ...
  function days_since($date1, $return_number = false) {
    ...
    $date1 = apply_filters('Flawless_F_days_since', $date1);
    ...
    if($days == 0) __('Today', 'flawless');
    elseif($days == 1) { return($future ? __("Tomorrow ",'flawless') : __("Yesterday ",'flawless')); }
    elseif($days > 1 && $days <= 6) { return ($future ? sprintf(__(" in %d days ",'flawless'), $days) : sprintf(__("%d days ago",'flawless'), $days)); }
  ...
  function add_log_page() {
    add_action('admin_menu', create_function('', "add_menu_page(__('Log','flawless'), __('Log','flawless'), 10, 'ud_log', array('Flawless_UI','show_log_page'));"));
  }
...
/**
  ...
  function json($success, $message) {
    ...
    // Log in database
    Flawless_F::log(__("Automatically logged failed JSON response: ",'flawless').$message);
   ...
   */
   function time_ago($time) {
    ...
    if($minutes_ago > 60) {
      return sprintf(__('%s hours ago','flawless'),round($minutes_ago / 60));
    } else {
      return sprintf(__('%s minutes ago','flawless'),$minutes_ago);
    }
...
endif; /* f(!class_exists('Flawless_F')): */
